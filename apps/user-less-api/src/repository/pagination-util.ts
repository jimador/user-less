...
const _calculateOffset = (pageNumber: number, pageSize: number): number =>
  (pageNumber - 1) * pageSize;

/**
 * Clip an array of {T} to a given page size at a given page number
 * @param items
 * @param pageOptions
...
export const getPage = <T>(items: T[], pageOptions: IPageInfo) => {
  ...
  if (pageNumber < 1) {
    throw new Error(
      `Page number must be greater than 1. Number: ${pageNumber}`
    );
  }
